...
def get_facerender_data(coeff_path, pic_path, first_coeff_path, audio_path,
    ...
    if 'full' not in preprocess.lower():
        ...
        # 1 70
        source_semantics_full = source_semantics_dict['coeff_3dmm_full'][:1, :70]
        ...
        # 1 70
        source_semantics_full = source_semantics_dict['coeff_3dmm_full'][:1, :73]
        generated_3dmm_full = generated_dict['coeff_3dmm_full'][:, :70]
    ...
    source_semantics_new_full = transform_semantic_1(
        source_semantics_full, semantic_radius)
    source_semantics_ts_full = torch.FloatTensor(
        source_semantics_new_full).unsqueeze(0)
    source_semantics_ts_full = source_semantics_ts_full.repeat(
        batch_size, 1, 1)
    data['source_semantics_full'] = source_semantics_ts_full

    # target
    generated_3dmm[:, :64] = generated_3dmm[:, :64] * expression_scale
    generated_3dmm_full[:, :64] = generated_3dmm_full[:,
                                                      :64] * expression_scale

    if 'full' in preprocess.lower():

        generated_3dmm = np.concatenate([generated_3dmm, np.repeat(
            source_semantics[:, 70:], generated_3dmm.shape[0], axis=0)], axis=1)
        generated_3dmm_full = np.concatenate([generated_3dmm_full, np.repeat(
            source_semantics_full[:, 70:], generated_3dmm_full.shape[0], axis=0)], axis=1)

    if still_mode:
        generated_3dmm[:, 64:] = np.repeat(
            source_semantics[:, 64:], generated_3dmm.shape[0], axis=0)
        generated_3dmm_full[:, 64:] = np.repeat(
            source_semantics_full[:, 64:], generated_3dmm_full.shape[0], axis=0)
        temp = 1
    ...
    for frame_idx in range(frame_num):
        ...
        target_semantics_full = transform_semantic_target(
            generated_3dmm_full, frame_idx, semantic_radius)
    ...
    # frame_num 70 semantic_radius*2+1
    target_semantics_np_full = np.array(target_semantics_list_full)
    target_semantics_np_full = target_semantics_np_full.reshape(
        batch_size, -1, target_semantics_np_full.shape[-2], target_semantics_np_full.shape[-1])
    data['target_semantics_list_full'] = torch.FloatTensor(
        target_semantics_np_full)
...
def transform_semantic_target(coeff_3dmm, frame_index, semantic_radius):
    ...
    seq = list(range(frame_index - semantic_radius,
               frame_index + semantic_radius+1))
...
def gen_camera_pose(camera_degree_list, frame_num, batch_size):
    ...
    for i, degree in enumerate(camera_degree_list[1:]):
        ...
        degree_step = degree_per_frame * \
            abs(degree-degree_last)/(degree-degree_last)
        new_degree_list = new_degree_list + \
            list(np.arange(degree_last, degree, degree_step))
