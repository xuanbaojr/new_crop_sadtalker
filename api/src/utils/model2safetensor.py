...
path_of_net_recon_model = os.path.join(
    current_root_path, 'checkpoints', 'epoch_20.pth')
...
free_view_checkpoint = '/apdcephfs/private_shadowcun/SadTalker/checkpoints/facevid2vid_' + \
    str(size)+'-model.pth.tar'
load_cpk_facevid2vid(free_view_checkpoint, kp_detector=kp_extractor,
                     generator=generator, he_estimator=he_estimator)

wav2lip_checkpoint = os.path.join(
    current_root_path, 'checkpoints', 'wav2lip.pth')
...
save_file(model.state_dict(), "checkpoints/SadTalker_V0.0.2_" +
          str(size)+".safetensors")

# test
load_cpk_facevid2vid_safetensor('checkpoints/SadTalker_V0.0.2_'+str(
    size)+'.safetensors', kp_detector=kp_extractor, generator=generator, he_estimator=None)
